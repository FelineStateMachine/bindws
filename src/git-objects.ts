...
import { DEFAULT_GIT_LIMITS, type GitLimits } from "./git-limits.ts";
...
export async function gitGraph(repository: GitRepository, roots: readonly string[], limits: Readonly<GitLimits> = DEFAULT_GIT_LIMITS): Promise<GitObject[]> {
  ...
  while (queue.length) {
    ...
    if (seen.size >= limits.maxTransactionObjects) throw new LimitError("Git graph object limit reached");
    ...
    if (!Number.isSafeInteger(info.size) || info.size < 0 || info.size > limits.maxObjectBytes || bytes + info.size > limits.maxTransactionRawBytes) throw new LimitError("Git graph byte limit reached");
    bytes += info.size;
    edges += info.links.length;
    if (edges > limits.maxGraphEdges) throw new LimitError("Git graph edge limit reached");
