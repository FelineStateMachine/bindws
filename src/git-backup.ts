...
import { DEFAULT_GIT_SQLITE_LIMITS as LIMITS } from "./git-sqlite.ts";
...
export function exportGitBackup(relay: Relay, reserve: (bytes: number, entries?: number) => boolean): GitBackup | string {
  const result: GitBackup = { format: "bind.ws/git-sqlite/1", repositories: [] };
  for (const row of gitSql<{ repository: string; compressed_bytes: number; metadata_bytes: number; object_count: number }>(relay, "SELECT repository,compressed_bytes,metadata_bytes,object_count FROM git_sqlite_meta ORDER BY repository LIMIT 12001").toArray()) {
    ...
    for (const object of gitSql<{ oid: string; type: GitObjectType; size: number }>(relay, "SELECT oid,type,raw_size AS size FROM git_sqlite_objects WHERE repository=? ORDER BY oid LIMIT 4097", row.repository).toArray()) {
      const chunks = gitSql<{ data: ArrayBuffer }>(relay, "SELECT data FROM git_sqlite_object_chunks WHERE repository=? AND oid=? ORDER BY chunk LIMIT 10", row.repository, object.oid).toArray().map(chunk => base64(new Uint8Array(chunk.data)));
      if (!reserve(0, chunks.length)) return "restricted: Git backup object limit reached";
      repo.objects.push({ ...object, chunks });
    }
    const refs = gitSql<{ name: string; oid: string }>(relay, "SELECT name,oid FROM git_sqlite_refs WHERE repository=? ORDER BY name LIMIT 1025", row.repository).toArray();
    repo.refs = Object.fromEntries(refs.map(ref => [ref.name, ref.oid]));
    repo.receipts = gitSql<{ id: string; sequence: number; requestHash: string; recordHash: string; packHash: string | null; updates: string }>(relay, "SELECT id,sequence,request_hash AS requestHash,record_hash AS recordHash,pack_hash AS packHash,updates FROM git_sqlite_receipts WHERE repository=? ORDER BY sequence LIMIT 12001", row.repository).toArray().map(receipt => ({ ...receipt, updates: JSON.parse(receipt.updates) as RefUpdate[] }));
...
export async function prepareGitBackup(input: unknown, maxEntries = 12000): Promise<PreparedGit[] | string> {
  ...
  try {
    ...
    for (const repo of backup.repositories) {
      ...
      for (const row of repo.objects) {
        if (!row || !hex.test(row.oid) || objects.has(row.oid) || !["blob", "tree", "commit", "tag"].includes(row.type) || !Number.isSafeInteger(row.size) || row.size < 0 || row.size > LIMITS.maxObjectBytes || !Array.isArray(row.chunks) || !row.chunks.length || row.chunks.length > 9) throw new Error();
