...
The alarm also runs succession, the dead-man's switch. `ownerSeenAt` in storage is refreshed by owner-signed actions (management calls, websocket AUTH, bridge requests, stored events), at most hourly. With an heir named, silence past the chosen delay puts a warning record in storage, mirrored in memory so NIP-11 can report `succession_pending`; the owner is notified weekly for thirty days; then `transferOwner` runs, both parties are notified and the handover is logged. Any owner action clears the warning.
