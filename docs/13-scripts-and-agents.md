...
- `gitstorage owner identifier`: an owner-only inventory of one accepted GRASP repository. The result reports compressed and raw Git object bytes, metadata bytes, ref and receipt counts, and the relay's shared physical SQLite size. The byte breakdown is diagnostic, not another fuel charge. It never deletes data.
- `deleteblob sha256`.
- `listdumps`, `dumpnow`, `deletedump name`.
- `backupnow [id]`, `listbackups`, `deletebackup id`: portable archives; owner NIP-98 downloads use `/backups/id`. Pass `{ "id": "name", "includeGit": false }` to `backupnow` when Git is too large for the bounded archive; recover large repositories separately with `git clone --mirror` or `git bundle`. POST the exact archive with a payload-bound NIP-98 signature to `/backups/preview`, then separately sign `/backups/restore` on a fresh target. Both require the archived owner key. Archives are capped at 8 MiB and 12,000 entries.
...
```
npm run relay check relay.jsonc                      # the schema and the relay's own parser, offline
npm run relay plan  relay.jsonc wss://name.bind.ws   # what applying would change; touches nothing
npm run relay push  relay.jsonc wss://name.bind.ws   # the plan, then the change
npm run relay pull  wss://name.bind.ws relay.json    # the relay as it is, into a file
```
