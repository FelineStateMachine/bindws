// Real HTTP and alarm reads pause at SQLite to expose the live instance's
...
describe("GRASP coordination", () => {
  ...
  it.each([
    ...
    await runInDurableObject(env.RELAY.getByName(host.split(".")[0]), async (relay) => {
      ...
      let held = false;
      const originalInfo = GitSqliteRepository.prototype.loadRefs;
      GitSqliteRepository.prototype.loadRefs = async function () {
        if (!held) {
          held = true;
          entered();
          await gate;
          if (outcome === "failure") throw new Error("injected SQLite read failure");
        }
        return originalInfo.call(this);
      };
      ...
      try {
        ...
        release();
        await active;
        GitSqliteRepository.prototype.loadRefs = originalInfo;
      }
    });
  });
});
