...
function doors(f: Fixture): { path: string; host?: string; method?: string; gated: boolean; blossom?: string }[] {
  return [
    ...
    { host: f.siteHost, path: "/.well-known/nostr.json?path=%2F", gated: true },
    { host: f.siteHost, path: "/.well-known/nostr.json?path=%2Findex.html", method: "HEAD", gated: true },
    { path: "/", gated: false },
    { path: "/people", gated: false },
    { path: "/.well-known/nostr.json", gated: false },
    { path: "/.well-known/nostr.json?name=nobodyq7", gated: false },
    { path: "/.well-known/nostr.json?path=%2F", gated: true },
    { path: `/.well-known/nostr.json?path=${encodeURIComponent(`/e/${f.noteId}`)}`, gated: false },
    { path: `/.well-known/nostr.json?path=${encodeURIComponent(`/a/${pk(f.eve)}/private`)}`, gated: false },
