...
describe("storage: retention and purge", () => {
  it("refuses events past a keep-for rule, sheds old ones daily, spares replaceable kinds from the catch-all, and purges on demand", async () => {
    ...
    expect(byKind).toEqual({ 1: 2, 7: 1, 0: 1, 13534: 1 }); // the roster is the relay's own
    expect(st.events).toBe(5);
    ...
    expect((await rpc(host, owner, "purgekind", null, 0)).result.deleted).toBe(1); // the fresh note; profile and roster stay
    expect((await rpc(host, owner, "storagestats")).result.kinds.map((k: any) => k.kind).sort()).toEqual([0, 13534]);
  });
});
