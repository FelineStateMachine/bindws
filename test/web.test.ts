...
describe("relay-signed notifications", () => {
  it("wraps a report for the owner, who alone can read it", async () => {
    ...
    expect((await rpc(host, owner, "getpolicy")).result.notify).toEqual({ reports: false, fuel: false, jobs: false });
    ...
    expect(set.result.notify).toEqual({ reports: true, fuel: false, jobs: false });
